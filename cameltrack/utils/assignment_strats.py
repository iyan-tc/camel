...
def hungarian_algorithm(td_sim_matrix, valid_tracks, valid_dets, sim_threshold=0.0, **kwargs):
    ...
    for b in range(B):
        if valid_tracks[b].sum() > 0 and valid_dets[b].sum() > 0:
            ...
            matched_td_indices = np.array(
                list(zip(valid_rows[row_idx], valid_cols[col_idx])))

            unmatched_trackers = [t.item() for t in valid_rows if
                                  t.item() not in set(valid_rows[row_idx].tolist())]
            ...
            if valid_tracks[b].sum() > 0:
                unmatched_trackers = torch.nonzero(valid_tracks[b]).squeeze(
                    dim=1).tolist()
...
def greedy_assignment(td_sim_matrix, threshold):
    ...
    pairs = [(td_sim_matrix[i, j], i, j) for i in range(td_sim_matrix.shape[0]) for j in
             range(td_sim_matrix.shape[1])]

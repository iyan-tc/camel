...
class FeatsDetDropout(BatchTransform):
    """
    Apply some dropout (replaced by zeros or random) on the either spatial or appearance features.
    ...
    def __init__(self, p_drop_app: float = 0.1, p_drop_spatial: float = 0.1,
    ...
    def _apply_dropout(self, features, targets):
        ...
        """
        drop_app_mask = (torch.rand(targets.shape, device=targets.device) < self.p_drop_app) & (~targets.isnan())
        drop_spatial_mask = (torch.rand(targets.shape, device=targets.device) < self.p_drop_spatial) & (~targets.isnan())

        # Ensure that only spatial OR appearance features is dropped
        dropped_both = drop_app_mask & drop_spatial_mask
        choose_one = (torch.rand(drop_app_mask.shape, device=targets.device) < 0.5)
        drop_app_mask[dropped_both] = choose_one[dropped_both]
        drop_spatial_mask[dropped_both] = ~choose_one[dropped_both]

        if self.dropout_type == "random":
            app_min, app_max = nanmin(features["embeddings"]), nanmax(features["embeddings"])
            app_values = torch.distributions.uniform.Uniform(app_min, app_max).sample(
                features["embeddings"][drop_app_mask].shape)

            bbox_min, bbox_max = nanmin(features["bbox_ltwh"]), nanmax(features["bbox_ltwh"])
            bbox_values = torch.distributions.uniform.Uniform(bbox_min, bbox_max).sample(
                features["bbox_ltwh"][drop_spatial_mask].shape)
            kps_min, kps_max = nanmin(features["keypoints_xyc"]), nanmax(features["keypoints_xyc"])
            kps_values = torch.distributions.uniform.Uniform(kps_min, kps_max).sample(
                features["keypoints_xyc"][drop_spatial_mask].shape)
        elif self.dropout_type == "zero":
            app_values = 0.
            bbox_values = 0.
            kps_values = 0.
        features["embeddings"][drop_app_mask] = app_values
        features["visibility_scores"][drop_app_mask] = 0.

        features["bbox_ltwh"][drop_spatial_mask] = bbox_values
        features["bbox_conf"][drop_spatial_mask] = 0.
        features["keypoints_xyc"][drop_spatial_mask][..., :2] = kps_values[..., :2]
        features["keypoints_xyc"][drop_spatial_mask][..., 2] = 0.
        return features
